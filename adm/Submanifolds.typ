...
=== ADM-Type Metric Decomposition in Coordinates <ADMDecompositionMetric>
...
+ *Anti-Symmetry in the First Pair* If the connection is metric-compatible (i.e. $nabla g =0$), the curvature operator acts on the metric as $R(X,Y)g = 0$.
  ...
  $
    0 & = R(X,Y) g(Z,W) \
      & = underbrace((R(X,Y)g), =0)(Z,W) + underbrace(g(R(X,Y)Z,W), =R(W,Z,X,Y)) + underbrace(g(R(X,Y)W,Z), =R(Z,W,X,Y)) \
      & =R(W,Z,X,Y) + R(Z,W,X,Y)
  $
...
To derive this identity, consider $X,Y in Gamma(T Sigma)$. We begin by expanding
$
...
$
  \(macron(cal(L))_beta gamma\)_(i j) &= lr((beta^k underbrace(mnabla_k gamma, =0)), size: #35%)#h(0em)_(i j) med + med (mnabla_i beta)#h(0em)^k gamma_(k j) + (mnabla_j beta)#h(0em)^k gamma_(i k)\
  &= (mnabla_i beta)#h(0em)_j med + med (mnabla_j beta)#h(0em)_i
$
Plugging this back into @lieFlowIntermResult[equation], we conclude that
$
  k_(i j) = - 1/(2 alpha) lr((diff_t gamma_(i j) - (mnabla_i beta)#h(0em)_j med - med (mnabla_j beta)#h(0em)_i ), size: #135%).
$<extCurvatureADMDecomp>
This characterises the extrinsic curvature's components entirely in terms of the time derivative of the induced metric, the symmetrised covariant derivative of the shift, and a scaling factor involving the lapse. Note that in particular, no time derivatives of $alpha$ nor $beta$ arise.
...
$
  k & = -iota^* (nabla n) = - iota^* (nabla dr) =  tensor(Gamma, +r, -mu nu)tensor(E,+mu,-i) tensor(E,+nu,-j) dy^i otimes dy^j  \
...
$
  k_(i j) = tensor(E,+mu,-i) tensor(E,+nu,-j) tensor(Gamma, +r, -mu nu) quad <=> quad k_(theta theta) = -r, quad k_(phi phi) = -r sin^2 theta.
$
...
$
Lastly, the decomposition of the identity into the orthogonal projectors $P$ and $Q$ as
$
  id = P + Q
$
...
$
  ...
  &attach(=, t: 2.) - g lr(( nabla_X underbrace((Q nabla_Y Z), =K(Y,Z)), W), size: #37%) + underbrace(g(Q nabla_X nabla_Y Z, W), =g lr((nabla_X nabla_Y Z, underbrace(Q W, =0)), size: #35%) = 0)\ 
...
+ The first term vanishes since $K(Y,Z) perp W$. In the second term, the first argument lies in $Gamma(N Sigma)$ such that only the normal part $Q nabla_X W = K(X,W)$ of the second argument contributes.
...
$
with respect to the velocities $diff_t alpha$, $diff_t beta_i$ as well as $diff_t gamma_(i j)$ associated to the degrees of freedom of the metric---the lapse $alpha$, the shift $beta_i$ and the induced metric $gamma_(i j)$.

To do this, we require a decomposition of the Ricci scalar $cal(R)$ (associated to the Levi-Civita connection) in terms of $alpha, beta_i, gamma_(i j)$. The goal of this section is to derive such a decomposition of the Ricci scalar known as the ADM Lagrangian density.
...
$
  E_perp = n^sharp, quad E_i = diff_i.
$
In this basis tangential components are cleanly separated from normal components. In particular, we have
$
  span{E_perp} = Gamma(N Sigma), quad span{E_i} = Gamma(T Sigma).
$
From @ADMDecompositionMetric we know that the inverse metric can be written as
$
  g^(-1) & = epsilon n^sharp otimes n^sharp + gamma^(i j) diff_i otimes diff_j \
         & = epsilon E_perp otimes E_perp + gamma^(i j) E_i otimes E_j.
$
This is a block-diagonal matrix with components
$
  g^(perp perp) = epsilon, quad g^(i j) = gamma^(i j), quad g^(perp i) = 0.
$
Hence, the Ricci scalar may be written as
$
  cal(R) & = g^(A C) g^(B D) R_(A B C D) = g^(k i) g^(ell j) R_(k ell i j) + 2 g^(perp perp) g^(A B) R_(perp A perp B) \
         & = gamma^(k i) gamma^(ell j) R_(k ell i j) + 2 tensor(R, +perp A, - perp A).
$<ambientRicciScalarExpansion1>
Let us now examine the two terms separately.

For the first term, we may refer back to the Gauss-Codazzi @gaussEqnHypersurfaceComponents[equation] we derived in the previous section. It states that the tangential components of the fully covariant ambient curvature, $R_(k ell i j)$, are given by
$
  R_(k ell i j) & = macron(R)_(k ell i j) - epsilon (k_(i k) k_(j ell) - k_(j k) k_(i ell)) \
$
Thus, the first term in the @ambientRicciScalarExpansion1[expansion] may be rewritten as
$
  gamma^(k i) gamma^(ell j) R_(k ell i j) &= underbrace(gamma^(k i) gamma^(ell j) macron(R)_(k ell i j), =macron(cal(R))) - epsilon lr((underbrace(gamma^(i k) k_(i k), =: k) underbrace(gamma^(ell j) k_(j ell), = k) - underbrace(k_(j k) gamma^(k i) k_(i ell) gamma^(ell j), =k_(i j) k^(i j))), size: #35%)\
  &= macron(cal(R)) - epsilon (k^2 - k_(i j) k^(i j)).
$
...
$
where $n = n_mu dx^mu$ is the normal 1-form. To derive $tensor(R, +perp mu, -perp mu)$, it is useful to introduce the shorthand notation
$
  Div X = tr_1^1 (nabla X) = (nabla_mu X)#h(0em)^mu
$
for the covariant divergence of a vector field $X in Gamma(T cal(M))$. With this definition, we may now continue deriving:
#bottom-number([
  $
    -epsilon tensor(R, +perp nu, -perp nu) &= -epsilon E^mu_perp tensor(R, +perp nu, -mu nu) = -epsilon n^mu tensor(R, +perp nu, -mu nu)\
    &= n^mu (nabla_mu nabla_nu n^sharp)#h(0em)^nu - n^mu (nabla_nu nabla_mu n^sharp)#h(0em)^nu\
    &= underbrace(nabla_mu lr((n^sharp otimes underbrace((nabla_nu n^sharp)#h(0em)^nu, =Div n^sharp)), size: #35%)#h(0em)^mu, = Div(n^sharp dot Div n^sharp)) - (nabla_mu n^sharp)#h(0em)^mu (nabla_nu n^sharp)#h(0em)^nu - underbrace(nabla_nu lr((underbrace(n^mu nabla_mu n^sharp, =nabla_(n^sharp)n^sharp)), size: #32%)#h(0em)^nu, =Div (nabla_(n^sharp) n^sharp)) + (nabla_nu n^sharp)#h(0em)^mu (nabla_mu n^sharp)#h(0em)^nu\
    &=Div(n^sharp dot Div n^sharp - nabla_(n^sharp) n^sharp) - (nabla_mu n^sharp)#h(0em)^mu (nabla_nu n^sharp)#h(0em)^nu+ (nabla_nu n^sharp)#h(0em)^mu (nabla_mu n^sharp)#h(0em)^nu
...
The other two terms are more interesting; we recall that for hypersurface foliations, the extrinsic curvature may be written as
$
  k(X,Y) = epsilon g(Y,-nabla_X n^sharp), quad X,Y in Gamma(T Sigma).
$
...
$
  tensor(R, +perp nu, -perp nu) = epsilon Div(nabla_(n^sharp) n^sharp - n^sharp dot Div n^sharp) + epsilon(k^2 - k_(i j) k^(i j)).
$
Hence, the ambient Ricci scalar is given by
$
...
$
  S_"ADM" [alpha,beta,gamma] = integral_cal(M) d^m x thin alpha sqrt(gamma) [macron(cal(R)) + epsilon (k^2 - k_(i j) k^(i j))] + "(boundary terms)".
$
